...
static RNInterval default_value_range(-0.2,0.2);
...
static int color_scheme = AFFINITY_COLOR;
...
ReadMeshFile(const char *filename)
{
  ...
  for (int i = 0; i < mesh->NVertices(); i++) {
    ...
    // If mesh has no faces, prefer rendering vertices
    if (mesh->NFaces() == 0) {
      show_vertices = 1;
      show_faces = 0;
    }
...
DrawMesh(int color_scheme)
{
  ...
  // Determine draw modes
  RNBoolean draw_vertices = show_vertices;
  RNBoolean draw_faces = (show_faces && (vbo_nfaces > 0) && (vbo_face_index_buffer > 0));

  // Set opengl modes
  if (draw_faces) glEnable(GL_LIGHTING);
...
static void
DrawAffinityColorBar(int color_scheme)
{
  // Show color bar only for affinity-based schemes
  if ((color_scheme != AFFINITY_COLOR) && (color_scheme != OVERLAY_COLOR)) return;
  if (mesh_affinities.IsEmpty()) return;
  ...
  // Determine layout
  int w = GLUTwindow_width;
  int h = GLUTwindow_height;
  RNScalar bar_margin = 40.0;
  RNScalar bar_width = 24.0;
  RNScalar bar_height = std::max(0.4 * h, 120.0);
  RNScalar x1 = w - bar_margin - bar_width;
  RNScalar x2 = w - bar_margin;
  RNScalar y1 = bar_margin;
  RNScalar y2 = y1 + bar_height;

  // Prepare 2D drawing state
  glDisable(GL_LIGHTING);
  ...
  // Draw gradient
  int steps = 64;
  RNScalar range = max_value - min_value;
  for (int i = 0; i < steps; ++i) {
    RNScalar t0 = (RNScalar)i / steps;
    RNScalar t1 = (RNScalar)(i + 1) / steps;
    RNScalar v0 = min_value + t0 * range;
    RNScalar v1 = min_value + t1 * range;
    RNScalar yy0 = y1 + t0 * bar_height;
    RNScalar yy1 = y1 + t1 * bar_height;
    RNRgb c0 = NormalizedColor(v0, color_scheme);
    RNRgb c1 = NormalizedColor(v1, color_scheme);
  ...
  char buffer[64];
  snprintf(buffer, sizeof(buffer), "%.2f", (double)max_value);
  viewer.DrawText((int)(x1 - 60), (int)(y2 - 6), buffer, font);
  snprintf(buffer, sizeof(buffer), "%.2f", (double)min_value);
  viewer.DrawText((int)(x1 - 60), (int)(y1), buffer, font);
}
...
UpdateMeshAffinities(void)
{
  ...
  // Update affinities
  RNScalar affinity_min = FLT_MAX;
  RNScalar affinity_max = -FLT_MAX;
  for (int m = 0; m < point_features.NEntries(); m++) {
    RNDenseMatrix *features = point_features.Kth(m);
    ...
    if (query_features.NValues() == features->NColumns()) {
      ...
      for (int i = 0; i < features->NRows(); i++) {
        ...
        // Update affinity using raw cosine similarity
        (*affinities)[i] = value;
        if (value < affinity_min) affinity_min = value;
        if (value > affinity_max) affinity_max = value;
      }
    }
    else {
      // Nonvalid query
      ...
      if (0 < affinity_min) affinity_min = 0;
      if (0 > affinity_max) affinity_max = 0;
    }
  }

  if (affinity_min < affinity_max) {
    value_range.SetMin(affinity_min);
    value_range.SetMax(affinity_max);
  }
  else {
    value_range.SetMin(affinity_min - 1e-3);
    value_range.SetMax(affinity_max + 1e-3);
  }
...
void GLUTRedraw(void)
{
  ...
  DrawAffinityColorBar(color_scheme);
